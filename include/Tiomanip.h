...
namespace Tstring{
	...
	private:
		...
		std::basic_ostream<_Elem>& PrintGuid(std::basic_ostream<_Elem>& iostr) const
		{
			...
			for (int i = 0; i<8; i++)	iostr << setw(2) << ((unsigned char*)&guid.Data4)[i];
	...
	private:
		...
		std::basic_ostream<_Elem>& PrintTabRight(std::basic_ostream<_Elem>& iostr)
		{
			...
			while (str.size() > LenghtStr)
			{
				...
				{
					if (Pos != npos) strLeft.insert(Pos, 1, _T(' '));
					Pos += 2;
				}
    ...
    // ColorBackground   - Цвет фона
     ...
     enum EnumColor{
			...
			dark_yellow = FOREGROUND_GREEN|FOREGROUND_RED,
			pink		= FOREGROUND_BLUE|FOREGROUND_RED|FOREGROUND_INTENSITY,
			dark_pink	= FOREGROUND_BLUE|FOREGROUND_RED,
	...
	private:
		...
		{
			if (radix < 2 || radix > 36) { iostr << TemplateTypeOfStr("Error Radix", _Elem); return iostr; }
			...
			if (radix > 10 && FlagUppercase) IntegerStr = GetUpperStr(IntegerStr);

			// вычислим количество нулей необходимых для дополнения числа до ширины поля			
			if (IsSigned) iostr << setw(0);
			...
			if (group != 0)
			{
				...
				{                                               //т.е. не стало меньше или равно 0
					...
					else IntegerStr.insert(i, TemplateTypeOfStr(" ", _Elem));

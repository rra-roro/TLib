...
namespace tlib::parser
{
      ...
      const TCHAR* const EOF_str = _T("___----EOF----___");
 ...
 */
      class command_line_t::GetOpt
      {
        private:
            ...
            // AnalizInitialisation() - предварительный анализ ARGV и вызов exchange() при неоходимсти
            // AnalizLongOption()     - распознавание длинных опций
            // AnalizShortOption()    - распознавание коротких опций

            bool AnalizInitialisation(vector<tstring>& argv);
            tlib::tstring AnalizLongOption(tstring& OptionStr, tstring_view ParamStr, vector<Option>::iterator* ptrLongOptInd);
            tlib::tstring AnalizShortOption(tstring& OptionStr, tstring_view ParamStr);
        ...
        public:
            ...
            tlib::tstring _getopt_internal(vector<tstring>& argv, vector<Option>::iterator* ptrLongOptInd);
...
*/
      ...
      void command_line_t::GetOpt::exchange(vector<tstring>& argv)
      {
      ...
      bool command_line_t::GetOpt::AnalizInitialisation(vector<tstring>& argv)
      {
      ...
      tlib::tstring command_line_t::GetOpt::AnalizLongOption(tstring& OptionStr, tstring_view ParamStr, vector<Option>::iterator* ptrLongOptInd)
      {
      ...
      tlib::tstring command_line_t::GetOpt::AnalizShortOption(tstring& ArgStr, tstring_view ParamStr)
      {
   ...
   (ф-ия AnalizInitialisation() перставит argv-элементы так, чтобы все неопции
...
*/
      ...
      tlib::tstring command_line_t::GetOpt::_getopt_internal(vector<tstring>& argv, vector<Option>::iterator* ptrLongOptInd)
      {
            this->optarg.clear();

            //----------------1-й IF----------------------
            //  AnalizInitialisation() анализирует ordering, и выполняет соответсвующии дейcтвия
            ...
            {
                  bool ret = AnalizInitialisation(argv);
                  if (!ret)
                        return EOF_str; // опций больше не венем EOF
            }
            ...
            try
            {
                          ...
                          short_opts_str.find(argv[optindex][1]) == npos))
                  {
                        ...
                        tlib::tstring ret = AnalizLongOption(argv[this->optindex], ParamStr, ptrLongOptInd);
      ...
      void command_line_t::SetShowError(bool ShowErrorFl)
      {
            GetOptObject->ShowErrorFlag = ShowErrorFl;
      }

      void command_line_t::SetSeparatorChar(TCHAR Ch)
      {
            GetOptObject->SeparatorChar = Ch;
      }

      void command_line_t::SetShortFormatOfArg(tlib::tstring Str)
      {
            GetOptObject->short_opts_str = Str;
      }

      void command_line_t::AddFormatOfArg(tlib::tstring name, _argtype has_arg)
      {
            GetOptObject->AddFormatOfArg(name, has_arg);
      }
      ...
      {
            tlib::tstring EnableOption; //После вызова _getopt_internal() содержит символ псевдоним найденной опции

            //Разбор коммандной строки
            while ((EnableOption = GetOptObject->_getopt_internal(ArgV_p, 0)) != EOF_str)
            {
                  //if (EnableOption != 0) //В случае ошибки возвращается 0
                  if (!EnableOption.empty()) //В случае ошибки возвращается 0
                  {
                        //Проверим нет ли в списке опций такой опции
                        auto ItrListItemArg = options.options_map.find(EnableOption);

                        if (ItrListItemArg != options.options_map.end())
                        {
                              //Если опция встретилась повторно, добавим новый параметр опции если он есть
                              if (GetOptObject->optarg != _T(""))
                                    ItrListItemArg->second.params.args.push_back(GetOptObject->optarg);
                        }
                        else
                        {
                              //Опция встретилась в первый раз, сохраним ее в списке
                              options_t::option_t TempItem;
                              TempItem.name_option = EnableOption;
                              TempItem.is_set = true;
                              if (GetOptObject->optarg != _T(""))
                                    TempItem.params.args.push_back(GetOptObject->optarg);
                              options.options_map[EnableOption] = TempItem;
                        }
                  }
                  else
                  {
                        //Сохраним ошибочные опции в списке ошибочных опций
                        unknown_otions.vector_non_options.push_back(GetOptObject->unknown_otion);
                  }
            }
            ...
            if (GetOptObject->optindex < ArgV_p.size())
            {
                  ...
                  non_options.vector_non_options.resize(ArgV_p.size() - GetOptObject->optindex, tstring());
                  for (size_t i = 0; (GetOptObject->optindex + i) < ArgV_p.size(); i++)
                  {
                        non_options.vector_non_options[i] = ArgV_p[GetOptObject->optindex + i];
                  }
            }
      }

...
int GetOpt::AnalizInitialisation(vector<tstring>& argv)
{
    ...
    // Если  у нас есть неопции в начале argv, то их нужно переставить после всех опций в argv
    {
            ...
            if (this->first_nonopt != this->last_nonopt && this->last_nonopt != this->optindex)
                ...
                if (this->last_nonopt != this->optindex) first_nonopt = optindex;
    ...
    // this->optindex будет указывать на первую не опцию
        ...
        if (this->optindex == argv.size())
        {
            ...
            if (this->first_nonopt != this->last_nonopt) this->optindex = this->first_nonopt;
...
int GetOpt::AnalizLongOption(tstring& OptionStr, tstring_view ParamStr, vector<Option>::iterator* ptrLongOptInd)
{
        ...
        for (auto ptrOption = this->long_opts_array.begin(); ptrOption != this->long_opts_array.end(); ptrOption++ )
        {
			...
			if(ptrOption->name.find(NameOptionStr) == 0)
            {
                ...
                else   ambig = true;                   // Второе не строгое соответствие найдено.
            }
        }
        ...
        if (ptrFoundOption != this->long_opts_array.end())
        {
                ...
                // определило опцию.
                if (ptrLongOptInd != NULL) *ptrLongOptInd = ptrFoundOption;
        ...
        if (short_opts_str[IndexShOtion + 1] == _T(':'))
        {
            if (short_opts_str[IndexShOtion + 2] == _T(':'))
            {
                ...
                if (!(nextchar != EndArgStr && nextchar[0] == _T('=')) &&
                    !(nextchar == EndArgStr && ParamStr != _T("") && !IsSeparatorTChar(ParamStr[0]))) optarg.clear();
...

{
        ...
        //----------------1-й IF----------------------
        //  AnalizInitialisation() анализирует ordering, и выполняет соответсвующии дейчтвия
        ...
        //  Если опция это '--' AnalizInitialisation() прерывапет сканирование
...
try{
        ...
        {			    
				...
				tstring_view ParamStr = ( (optindex+1) < argv.size() &&
					                      ...
					                      )? argv[this->optindex+1]: empty_param;
...
template void ParserCommandLine::Parser(int argc, wchar_t* argv[]);
...
void ParserCommandLine::Parser(const std::basic_string<_Elem>& ArgV_str, bool bProgramName)
{
	...
	if (bProgramName == false) ArgV_p.push_back(_T(""));
...
template void ParserCommandLine::Parser(const std::wstring& ArgV_str, bool bProgramName);

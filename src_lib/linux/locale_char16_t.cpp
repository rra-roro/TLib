    ...
    _M_put(char16_t* __s, size_t __maxlen, const char16_t* __format, const tm* __tm) const throw()
{
      ...
      const size_t __len = wcsftime_l(tmp.data(), __maxlen, u16str_wstr(__format).c_str(), __tm, _M_c_locale_timepunct);
      // Make sure __s is null terminated.
      if (__len == 0)
            tmp[0] = L'\0';
      wstr_u16str(tmp).copy(__s, __len);
      __s[__len] = u'\0';
}

#define CHAR16_LANGINFO(M, FMT)                                    \
      holder = u8str_u16str(nl_langinfo_l(FMT, (locale_t)__cloc)); \
...
void moneypunct<char16_t, true>::_M_initialize_moneypunct(__c_locale __cloc, const char*)
{
      ...
      else
      {
            ...
            __try
            {
                  ...
                  if (__cpossign.size())
                  {
                        u16string tmp = u8str_u16str(__cpossign);
                  ...
                  {
                        u16string tmp = u8str_u16str(__cnegsign);
                  ...
                  if (__ccurr.size())
                  {
                        u16string tmp = u8str_u16str(__ccurr);
...
///////////////
//std::locale::id tlib::inner_impl::ctype<char16_t>::id{};
